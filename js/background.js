...
chrome.storage.local.get(
  ["username", "allowNotification", "notification", "live"],
  res => {
...
allowNotif.onchange = that => {
  ...
  for (var elem of ListStreams.children) {
    if (notifON)
      elem.getElementsByClassName("notifdiv")[0].firstChild.src =
        "../img/alarmON.png";
    else
      elem.getElementsByClassName("notifdiv")[0].firstChild.src =
        "../img/alarmOFF.png";
...
window.onload = () => {
  chrome.storage.local.get("TwitchUserName", r => {
    if (r.TwitchUserName === undefined) r.TwitchUserName = "Twitch";
    username.value = r.TwitchUserName;
    Update(r.TwitchUserName);
  });
  chrome.storage.local.get("TwitchToolNotif", r => {
    if (r.TwitchToolNotif === undefined) {
      r.TwitchToolNotif = false;
      //Disable alter by default
    }
    ...
    chrome.storage.local.get("streamer", r => {
      val.forEach((Streamer, i) => {
        if (r.streamer != undefined && r.streamer.indexOf(Streamer.name) !== -1)
          ListStreams.children[i].getElementsByClassName(
            "notifdiv"
          )[0].firstChild.src =
            "../img/alarmOFF.png";
      }, this);
...
const BuildList = Streamer => {
  ...
  if (Streamer.isLive) {
    spanName.textContent +=
      " [LIVE] on " + Streamer.game + " (" + Streamer.viewers + " viewers)";
